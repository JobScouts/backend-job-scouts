...
router.post("/", async (req, res, next) => {
  try {
    // Get the user_id of the logged-in user from the authentication mechanism
    const user_id = req.user_id; // Replace 'user_id' with the actual way you obtain the user_id

    let { job_title, employer_name, employer_logo, employer_website, job_highlights, job_apply_link, sub } = req.body;
    
    let sql = 'INSERT INTO myjobs (job_title, employer_name, employer_logo, employer_website, job_highlights, job_apply_link, sub) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id';
    const result = await client.query(sql, [job_title, employer_name, employer_logo, employer_website, job_highlights, job_apply_link, sub]);

    const job_id = result.rows[0].id; // Get the auto-incremented job_id from the INSERT query

    // Store the user_id and job_id in the user_jobs table to connect them
    sql = 'INSERT INTO user_jobs (user_id, job_id) VALUES ($1, $2)';
    await client.query(sql, [user_id, job_id]);

    res.status(201).send(`Job '${job_title}' added to myjobs table and connected with user id:${user_id}`);
  } catch (error) {
